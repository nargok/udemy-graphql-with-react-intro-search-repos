        ...
        onClick={
          () => addOrRemoveStar({
            ...
            update: (store, {data: { addStar, removeStar }}) => {
              const { starrable } = addStar || removeStar; // addStarかremoveStarのどちらかが得られる
              ...
              const newEdges = edges.map(edge => {
                if (edge.node.id === node.id) { // ボタン押したリポジトリのstarを書き換える
                  const totalCount = edge.node.stargazers.totalCount;
                  const diff = starrable.viwerHasStarred ? 1 : -1;
...
class App extends Component {
  ...
  handleSubmit(event) {
    event.preventDefault(); // submitイベントを無効にする
